...
```{r}
paste("Goodbye", "former students")
```

(@) Can you explain what happened in 2-3 sentences? 

> I used 'paste()' to string two character vectors together.This is the same as using '+' to combine two numbers, just a different method.
...
> The code successfully decides whether the statements above are TRUE or FALSE.
...
> Three names of the presidents were stored within myvalue, and from it I learned how to determine the number of items within the vector, how to get the value from a certain number, how to create a vector using numbers, and how to calculate using my.numbers.
...
> It took from the value of the third item in my.numbers (which was six) and mutiplied it by two, getting 12.
...
```
```{r}
planets <- c("Earth", "Venus", "Mars" , "Mercury" , "Jupiter")
planets
```
...
It shows once again that there is no documentation for `floor()`
...
> I'd assume it is determining all the values that could be in x and y.


## Loading Dataframes from Packages

Our class's package is called `DigitalMethodsData`. This package isn't on CRAN — it lives on [GitHub](https://github.com/lukeave/DigitalMethodsData), which is where a lot of research software does. Normally you'd install it with a helper package like pak. But you already have it: ``renv::restore()`` read ``renv.lock``, saw the GitHub entry, and installed the package by pointing it to the commit it was first built on. That's the point of the lockfile. Let's just confirm it's there.
```{r}
...
> I was confused on how I could find the last six rows when I wasn't told the proper command, but then trying tails popped into my head> If that didn't end up working like it 
did, I would've tried feet.
...
> Gayguides is what we're pulling from.The money symbol allows us to get as segment from the dataframe. The 500 is the five hundreth item on the list, so If i instead put 200, I'd pull the up two hundreth item on the list.
...
> I learned how to work with numeric values, how to string together sentences, to figure out whether a statement is true or false, something with tibbles and vectors that i didn't entirely understand but seemed to succeed with anyways. I learned how to import and pull from a dataset in a variety of ways.
